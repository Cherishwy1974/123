...
# -*- coding: utf-8 -*-
"""
Final fix for startAutoPlay function - use simpler string replacement
"""

import os

def fix_autoplay_simple(file_path):
    """Replace the old startAutoPlay with new one using simple string matching"""
    ...
    if old_func_start in content:
        ...
        print(f"⚠️ Old function pattern not found in: {file_path}")
        return False
...
def main():
    ...
    for filename in files:
        ...
        if os.path.exists(file_path):
            try:
                if fix_autoplay_simple(file_path):
